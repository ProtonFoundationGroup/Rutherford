...
export default {
  ...
  beforeCreate () {
    ...
    ipc.on(Types.RUTHERFORD_LOG_DOWNLOADED, (event, state) => {
      this.$q.notify({ message: this.$t('notify.rutherford_downloaded'), color: 'primary', timeout: 2000 })
    })

    ipc.on(Types.RUTHERFORD_RELAUCH, (event, state) => {
  ...
  destroyed () {
    [
      ...
      Types.SWITCH_LAN,
      Types.SYNC_CONTRACT,
      Types.RUTHERFORD_LOG_DOWNLOADED,
      Types.RUTHERFORD_RELAUCH
    ].forEach(channel => {
