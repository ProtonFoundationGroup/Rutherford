...
class NodeSync extends EventEmitter {
  constructor () {
    ...
    ProtonNode.on('state', _.bind(this._onNodeStateChanged, this))
  }
  ...
  start () {
    ...
    this._syncPromise = Q.try(() => {
      if (!(ProtonNode.state === ProtonNode.STATES.CONNECTED)) {
        throw new Error('Cannot sync - Proton node not yet connected')
      }
  ...
  _onNodeStateChanged (state) {
    switch (state) { // eslint-disable-line default-case
      ...
      case ProtonNode.STATES.STOPPING:
        log.info('Proton node stopping, so stop sync')

        this.stop()
        break
      // auto-sync whenever node gets connected
      case ProtonNode.STATES.CONNECTED:
        log.info('Proton node connected, re-start sync')
