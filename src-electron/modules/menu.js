...
import zh from './i18n/rutherford.zh.i18n.json'
import en from './i18n/rutherford.en.i18n.json'
import { Types } from '../modules/ipc/types'
import path from 'path'
import fs from 'fs'
import settings from './settings'
import protonNode from '../modules/protonNode'
...
class RutherfordMenu {
  ...
  kickStart (restart) {
    ...
    Q.resolve()
      .then(() => {
        observeManager.stop()
        global.windows.broadcast(Types.RUTHERFORD_RELAUCH)
        return clientBinaryManager.init(true)
      }).then(() => {
        if (restart) {
          return protonNode.restart(settings.nodeType, settings.network, settings.syncmode)
        }
        return protonNode.init()
      })
      .then(() => {
        log.info('Proton node restarted.')
      })
  ...
  create () {
    ...
    const debugSubmenus = [
      ...
      // {type: 'separator'},
      {
        ...
        click: () => {
          let filename = path.resolve(app.getPath('userData'), 'rutherford.log')
          fs.writeFileSync(path.resolve(app.getPath('desktop'), 'rutherford.log'), fs.readFileSync(filename))
          global.windows.broadcast(Types.RUTHERFORD_LOG_DOWNLOADED)
        }
      },
      {type: 'separator'},
      {
